...
class Generator(object):
    def __init__(self, vocab_size, batch_size, emb_dim, hidden_dim,
        ...
        self.h0 = tf.stack([self.h0, self.h0])  # 初始化LSTM的h和c的
        # gen_o seq results(proba), gen_x seq results(index)
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            h_t = self.g_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple  （h,c）
            o_t = self.g_output_unit(h_t)  # batch x vocab , return logits , just use h in tuple(h,c)
            log_prob = tf.log(tf.nn.softmax(o_t))
            # multinomial按照概率进行采样
            next_token = tf.cast(tf.reshape(tf.multinomial(log_prob, 1), [self.batch_size]), tf.int32)
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch x emb_dim
            # tf.reduce_sum()：把rnn输出的o_t中根据next_token采样结果对应位置的概率拿到
        ...
        _, _, _, self.gen_o, self.gen_x = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings, self.start_token),  # batch size初始化的时候每句话的起始
                       self.h0,  # 初始化LSTM的h和c的
                       gen_o,
                       gen_x)
        )
        ...
        self.g_loss = -tf.reduce_sum(
            tf.reduce_sum(
                ...
                tf.log(tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_emb]), 1e-20, 1.0)), 1)
    ...
    def create_recurrent_unit(self, params):
        ...
        # input gate
        self.Wi = tf.Variable(self.init_matrix([self.emb_dim, self.hidden_dim]))  # for xt
        self.Ui = tf.Variable(self.init_matrix([self.hidden_dim, self.hidden_dim]))  # for ht-1
